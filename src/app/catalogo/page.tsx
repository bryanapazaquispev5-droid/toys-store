          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs sm:text-sm text-slate-500 mt-1">
                Encuentra el regalo ideal para cada edad. Pide directamente por WhatsApp.
              </p>
            </div>
            ...
            <div className="w-full md:w-80 lg:w-96 relative">
              ...
              <input
                ...
                className="w-full pl-11 pr-10 py-3 text-sm bg-slate-100 hover:bg-slate-50 focus:bg-white rounded-2xl border border-slate-200 focus:border-rose-400 outline-hidden transition-all text-slate-900 placeholder-slate-400 shadow-2xs"
              />
              {search && (
                <button
                  onClick={() => setSearch('')}
      ...
      <main className="max-w-7xl 2xl:max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full flex-1">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="text-xs font-semibold bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-slate-700 outline-hidden focus:border-rose-400 transition-colors cursor-pointer"
              >
                {AGE_RANGES.map((age) => (
            ...
            {/* In stock toggle */}
            <label className="flex items-center gap-2 text-xs font-bold text-slate-700 bg-slate-50 border border-slate-200 px-3 py-2 rounded-xl cursor-pointer hover:bg-slate-100 transition-colors">
              <input
                type="checkbox"
                checked={onlyInStock}
        ...
        <div className="mt-4 mb-6 flex items-center justify-between">
          <p className="text-xs sm:text-sm font-semibold text-slate-600">
            Mostrando <span className="text-slate-900 font-extrabold">{filteredProducts.length}</span> juguetes encontrados
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 gap-6">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="bg-white rounded-3xl p-4 border border-slate-200 shadow-xs animate-pulse">
          ...
          <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 shadow-xs max-w-lg mx-auto my-12">
            ...
            <h3 className="text-xl font-bold text-slate-800">No encontramos juguetes</h3>
            ...
            <button
              onClick={resetAllFilters}
              className="mt-6 px-6 py-2.5 rounded-2xl bg-rose-500 hover:bg-rose-600 text-white text-xs font-bold transition-all shadow-md shadow-rose-200 cursor-pointer"
            >
              Restablecer Filtros
            </button>
          </div>
            ...
            {filteredProducts.map((product) => {
              ...
              return (
                <div
                  ...
                  className="bg-white p-4 sm:p-5 rounded-3xl border border-slate-200 hover:border-rose-200 shadow-xs hover:shadow-md transition-all flex flex-col sm:flex-row items-center gap-5 cursor-pointer group"
                >
                  ...
                  <div className="w-full sm:w-48 flex flex-col gap-2 shrink-0">
                    <button
                      ...
                      className="w-full py-2.5 px-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow-xs transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                    >
                    ...
                    <button
                      ...
                      className={`w-full py-2.5 px-3 rounded-xl font-bold text-xs transition-colors flex items-center justify-center gap-1.5 cursor-pointer ${
                        product.in_stock
