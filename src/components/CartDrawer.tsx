    ...
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-md bg-white shadow-xl flex flex-col">
          <div className="p-4 sm:p-5 border-b border-slate-200 flex items-center justify-between bg-sky-50/50">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
            {items.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center py-12">
                <div className="w-16 h-16 rounded-full bg-sky-50 flex items-center justify-center text-3xl mb-3 border border-sky-100">
                  🧸
                </div>
                ...
                <button
                  onClick={closeCart}
                  className="mt-5 px-6 py-2.5 rounded-full bg-sky-600 text-white text-xs font-bold hover:bg-sky-700 shadow-xs transition-colors cursor-pointer"
                >
                  Ver Catálogo
                </button>
              </div>
              ...
              <>
                <div className="space-y-3">
                  {items.map(({ product, quantity }) => (
                    <div
                      key={product.id}
                      className="flex items-center gap-3 p-3 rounded-lg border border-slate-200 bg-slate-50 hover:bg-slate-100/60 transition-colors"
                    >
                      <img
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          <div className="flex items-center border border-slate-300 bg-white rounded-md overflow-hidden">
                            <button
                              ...
                              className="p-1 text-slate-600 hover:text-pink-600 hover:bg-slate-100 transition-colors cursor-pointer"
                            >
                            ...
                            <button
                              onClick={() => updateQuantity(product.id, quantity + 1)}
                              className="p-1 text-slate-600 hover:text-emerald-600 hover:bg-slate-100 transition-colors cursor-pointer"
                            >
                ...
                <div className="mt-6 pt-4 border-t border-slate-200 space-y-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full text-xs px-3 py-2 bg-slate-50 rounded-md border border-slate-200 focus:bg-white focus:border-sky-500 outline-hidden transition-all text-slate-800"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="Ej. ¿Tienen empaque para regalo?"
                      value={note}
          ...
          {items.length > 0 && (
            <div className="p-4 sm:p-5 border-t border-slate-200 bg-slate-50 space-y-3">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-3.5 px-4 rounded-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm shadow-xs transition-colors cursor-pointer"
              >
              ...
              <div className="flex items-center justify-between text-[11px] text-slate-400 pt-1">
                ...
                <button
                  onClick={clearCart}
                  className="hover:text-pink-600 underline transition-colors cursor-pointer"
                >
                  Vaciar carrito
                </button>
              </div>
